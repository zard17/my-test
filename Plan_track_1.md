...
## Phase 0: Data Acquisition (Figma REST API)
*목표: Track 1 실행을 위한 원시 데이터 및 검증용 이미지 수집*

### ### 0.1 API Client 구현 (`apps/track1-cli/src/figma_client.ts`)
- `axios` 또는 `node-fetch`를 사용하여 Figma REST API 래퍼 클래스 구현.
- **GetNodeData:** 특정 Node ID의 JSON 데이터를 가져와 로컬 캐시(`.cache/*.json`)에 저장.
- **GetNodeImage:** Visual Test 비교군으로 쓸 원본 디자인 PNG URL 획득 및 다운로드.

### ### 0.2 Environment Configuration
- `.env` 파일을 통해 `FIGMA_ACCESS_TOKEN` 및 `TEST_FILE_KEY` 관리.

## Phase 1: Shared Core & IR Schema
목표: 플랫폼에 의존성 없는 순수 TypeScript 데이터 변환 레이어 구축

### ### 1.1 Standard IR 인터페이스 정의 (packages/common/types.ts)
...
### ### 1.2 Core Parser 구현 (packages/core/parser.ts)
...
## Phase 2: Track 1 Execution (LLM-First)
목표: LLM을 활용한 코드 생성 및 품질 실험

### ### 2.1 CLI Fetcher & LLM Client
...
### ### 2.5.2 Human-Reviewed Rule Extraction
